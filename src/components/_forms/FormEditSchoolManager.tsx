...
export default function FormEditSchoolManager() {
...
const [contactId, setContactId] = useState<number | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      //  constante que preparar o payload com os dados formatados
      const payload = {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const gestorData = response.data.data.find(c => c.contactType === "GESTOR");
  
      if (!gestorData) {
        alert("Gestor não encontrado com o CPF informado.");
        return;
      }
  
      const address = parseAddress(gestorData.address || "");
  
      setFormData({
        fullName: gestorData.fullName || "",
        email: gestorData.email || "",
        documentId: gestorData.documentId || "",
        phonePrimary: gestorData.phonePrimary || "",
        phoneSecondary: gestorData.phoneSecondary || "",
        contactType: gestorData.contactType || "GESTOR",
        birthDate: gestorData.birthDate || "",
        status: gestorData.status || "ATIVO",
        gender: gestorData.gender || "MASCULINO",
        accountId: gestorData.accountId || 0,
      ...
      setContactId(gestorData.contactId);
      alert("Gestor encontrado!");
    } catch (error) {
      console.error("Erro ao buscar gestor:", error);
      if (isAxiosError(error)) {
        const errorResponse = error.response?.data as ErrorResponse;
        alert(errorResponse?.message || "Erro ao buscar gestor");
      } else {
        alert("Erro inesperado ao buscar gestor");
      }
    } finally {
  ...
  return (
    <form 
      ...
      className="w-full max-w-7xl mx-auto p-4 h-[calc(110vh-2rem)] flex flex-col"

    >
      <div className="bg-white rounded-lg shadow-md p-2 overflow-y-auto" style={{ maxHeight: 'calc(85vh - 4rem)' }}>
        <h2 className="text-xl font-bold text-[#155DFC] mb-6">Cadastro de Gestor</h2>
        
        <div className="mb-6">
          <p className="text-base font-medium text-[#155DFC]">Buscar Gestor Existente</p>
